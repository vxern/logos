...
import { EntryRequest } from 'logos/src/database/structs/mod.ts';
import {
	CacheAdapter,
	DatabaseAdapters,
	dispatchQuery,
...
import { EntryRequestIndexes } from 'logos/src/database/indexes.ts';
...
const adapter: DatabaseAdapters['entryRequests'] = {
	...
	update: async (client, entryRequest) => {
		const document = await dispatchQuery<EntryRequest>(client, $.Update(entryRequest.ref, entryRequest));
