import { ClientEvents, Collector, Guild, GuildTextChannel } from "../../../deps.ts";
...
class LoggingController extends Controller {
  ...
  async setupChannel(guild: Guild): Promise<void> {
    ...
    this.channel = channels.find((channel) => channel.name.includes("journal")) as GuildTextChannel | undefined;
  }

  async startListening(guild: Guild): Promise<void> {
    if (!this.channel) {
      console.error(`Failed to set up log service for guild '${guild.name!}': No journal channel found.`);
      return;
    }
    ...
    for (const [event, entry] of Object.entries(generators)) {
      const collector = new Collector({
        event: event,
        client: guild.client,
        filter: (...args) => (entry.filter as (...args: any[]) => boolean)(guild, ...args),
      });
