...
async function word(interaction: Interaction): Promise<void> {
  ...
  if (dictionaries.length === 0) {
    console.log(`${interaction.user.username} attempted to look up '${word}' in ${capitalise(language)}, but there are no available dictionaries for the language.`);
    response.editResponse({
      embeds: [{
        title: "No available dictionaries.",
        description: `There are no dictionary adapters installed for the ${capitalise(language)} language.`,
  ...
  dictionaries.map((dictionary) => dictionary.lookup(word, language).then((result) => {
    entry = {...result, ...entry};

    const fields = toFields(entry, {verbose: verbose});
    const hasEntry = fields.length > 0;
    if (fields.length === 0) {
      console.log(`${interaction.user.username} attempted to look up '${word}' in ${capitalise(language)}, but no results came up.`);
    }

    response.editResponse({
      embeds: [{
        title: entry.headword,
        description: !hasEntry ? `No results found for '${entry.headword}'.` : undefined,
        fields: fields,
