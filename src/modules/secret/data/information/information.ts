	...
	rules: {
		...
		generateEmbed: async (guild) => {
			...
			for (const [title, generateRule] of Object.entries(rules)) {
				...
				fields.push({
					name: `💠  ${bold(title.toUpperCase())}  ~  TLDR: ${italic(rule.summary)}`,
