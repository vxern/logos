...
import {getPartOfSpeech, PartOfSpeech} from "logos:constants/parts-of-speech";
import {DictionaryAdapter, DictionaryEntry} from "logos/adapters/dictionaries/adapter";
...
class DexonlineAdapter extends DictionaryAdapter<Dexonline.Results> {
    ...
    parse(
        ...
        for (const result of results.synthesis) {
            ...
            const partOfSpeech = getPartOfSpeech({ terms: { exact: result.type, approximate: topicWord }, learningLanguage: "Romanian" });
            ...
            entries.push({
                ...
                sources: [[constants.links.dexonlineDefinition(result.lemma), constants.licences.dictionaries.dexonline]],
            });
        }
        ...
        for (const { table } of results.inflection) {
            ...
            for (const partOfSpeechRaw of partsOfSpeechRaw) {
                ...
                const partOfSpeech = getPartOfSpeech({ terms: { exact: partOfSpeechRaw, approximate: topicWord }, learningLanguage: "Romanian" });
            ...
            entry.inflectionTable = this.tableRowsToFields(client, interaction, entry.partOfSpeech[0], table);
        }
    ...
    private tableRowsToFields(
        ...
        switch (partOfSpeech) {
            case "pronoun": {
                return this.pronounTableToFields(client, interaction, table);
            }
            case "noun": {
                return this.nounTableToFields(client, interaction, table);
            }
            case "verb": {
                return this.verbTableToFields(client, interaction, table);
            }
            case "adjective": {
                return this.adjectiveTableToFields(client, interaction, table);
            }
            case "determiner": {
                return this.determinerTableToFields(client, interaction, table);
            }
        }
    ...
    private pronounTableToFields(
        ...
        const [nominativeAccusative, genitiveDative] = table.slice(1).map((columns) => columns.slice(2).join(", ")).toChunked(2);
        if (nominativeAccusative === undefined || genitiveDative === undefined) {
            return undefined;
        }

        const strings = constants.contexts.dexonlinePronoun({ localise: client.localise.bind(client), locale: interaction.parameters.show ? interaction.guildLocale : interaction.locale });
        return [
            {
                title: strings.title,
                fields: [
    ...
    private nounTableToFields(
        ...
        const [nominativeAccusative, genitiveDative, vocative] = table.slice(1).map((columns) => columns.slice(2)).toChunked(2);
        if (nominativeAccusative === undefined || genitiveDative === undefined) {
            return undefined;
        }
        ...
        const strings = constants.contexts.dexonlineNoun({ localise: client.localise.bind(client), locale: interaction.locale });
        const numberColumn = {
            name: constants.special.meta.whitespace,
            value: `**${strings.singular}**\n**${strings.plural}**`,
            inline: true,
    ...
    private verbTableToFields(
        ...
        const presumptivePresent = DexonlineAdapter.#presumptiveAuxiliaryForms.map((auxiliary) => `${auxiliary} ${infinitive}`);
        ...
        const presumptivePerfect = DexonlineAdapter.#presumptiveAuxiliaryForms.map((auxiliary) => `${auxiliary} fi ${pastParticiple}`);

        const indicativePerfect = DexonlineAdapter.#pastAuxiliaryForms.map((auxiliary) => `${auxiliary} ${pastParticiple}`);
        const indicativeFutureCertain = DexonlineAdapter.#futureAuxiliaryForms.map((auxiliary) => `${auxiliary} ${infinitive}`);
        ...
        const indicativeFuturePerfect = DexonlineAdapter.#futureAuxiliaryForms.map((auxiliary) => `${auxiliary} fi ${pastParticiple}`);

        const conditionalPresent = DexonlineAdapter.#conditionalAuxiliaryForms.map((auxiliary) => `${auxiliary} ${infinitive}`);
        const conditionalPerfect = DexonlineAdapter.#conditionalAuxiliaryForms.map((auxiliary) => `${auxiliary} fi ${pastParticiple}`);

        const strings = constants.contexts.dexonlineVerb({ localise: client.localise.bind(client), locale: interaction.locale });
        return [
            {
    ...
    private adjectiveTableToFields(
        ...
        const [nominativeAccusative, genitiveDative] = table.slice(2).map((columns) => columns.slice(2, 8)).toChunked(2);
        if (nominativeAccusative === undefined || genitiveDative === undefined) {
            return undefined;
        }

        const strings = constants.contexts.dexonlineAdjective({ localise: client.localise.bind(client), locale: interaction.locale });
        return [
            {
                title: strings.title,
                fields: [
    ...
    private determinerTableToFields(
        ...
        const strings = constants.contexts.dexonlineDeterminer({ localise: client.localise.bind(client), locale: interaction.locale });
        return [
            {
                title: strings.title,
                fields: [
