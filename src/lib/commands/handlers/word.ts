...
async function handleFindWordAutocomplete(client: Client, interaction: Logos.Interaction<any, { language: string | undefined }>): Promise<void> {
    ...
    if (languageQueryTrimmed === undefined || languageQueryTrimmed.length === 0) {
        const strings = constants.contexts.autocompleteLanguage({ localise: client.localise.bind(client), locale: interaction.locale });
...
async function handleFindWord(client: Client, interaction: Logos.Interaction<any, { word: string; language: string | undefined; verbose: boolean | undefined }>): Promise<void> {
    ...
    const learningLanguage = interaction.parameters.language !== undefined ? interaction.parameters.language : interaction.learningLanguage;
    ...
    if (dictionaries === undefined) {
        const strings = constants.contexts.noDictionaryAdapters({ localise: client.localise.bind(client), locale: interaction.locale });
        await client.reply(interaction, {
            embeds: [
                {
                    title: strings.title,
    ...
    client.log.info(
        `Looking up the word '${interaction.parameters.word}' from ${dictionaries.length} dictionaries as requested by ${client.diagnostics.user(
            interaction.user,
        )} on ${guild.name}...`,
    );
    ...
    for (const dictionary of dictionaries) {
        ...
        const entries = await dictionary.getEntries(client, interaction.parameters.word, learningLanguage);
    ...
    if (entriesByPartOfSpeech.size === 0) {
        const strings = constants.contexts.noResults({ localise: client.localise.bind(client), locale: interaction.displayLocale });
    ...
    const showButton = interaction.parameters.show ? undefined : client.interactionRepetitionService.getShowButton(interaction);
...
async function generateButtons(
    ...
    if (entry.inflectionTable !== undefined) {
        const strings = constants.contexts.inflectionView({ localise: client.localise.bind(client), locale: interaction.displayLocale });
        row.push({
            type: Discord.MessageComponentTypes.Button,
...
function entryToEmbeds(
    ...
    if (entry.partOfSpeech === undefined) {
        const strings = constants.contexts.partOfSpeechUnknown({ localise: client.localise.bind(client), locale: interaction.displayLocale });
        ...
        if (detected === "unknown") {
            ...
            const strings = constants.contexts.partOfSpeech({ localise: client.localise.bind(client), locale: interaction.displayLocale });
    ...
    if (entry.nativeDefinitions !== undefined && entry.nativeDefinitions.length !== 0) {
        ...
        if (verbose) {
            const strings = constants.contexts.nativeDefinitionsForWord({ localise: client.localise.bind(client), locale: interaction.displayLocale });
            embeds.push({
            ...
            const strings = constants.contexts.nativeDefinitions({ localise: client.localise.bind(client), locale: interaction.displayLocale })
            fields.push({
    ...
    if (entry.definitions !== undefined && entry.definitions.length !== 0) {
        ...
        if (verbose) {
            const strings = constants.contexts.definitionsForWord({ localise: client.localise.bind(client), locale: interaction.displayLocale });
            embeds.push({
            ...
            const strings = constants.contexts.definitions({ localise: client.localise.bind(client), locale: interaction.displayLocale });
            fields.push({
    ...
    if (entry.expressions !== undefined && entry.expressions.length !== 0) {
        ...
        const strings = constants.contexts.expressions({ localise: client.localise.bind(client), locale: interaction.displayLocale });
        if (verbose) {
            embeds.push({
    ...
    if (entry.etymologies !== undefined && entry.etymologies.length !== 0) {
        ...
        const strings = constants.contexts.etymology({ localise: client.localise.bind(client), locale: interaction.displayLocale });
        if (verbose) {
            embeds.push({
            ...
            fields.push({ name: `${constants.emojis.word.etymology} ${strings.etymology}`, value: trim(etymology, 1024) });
        }
    }

    const strings = constants.contexts.sourcedFromDictionaries({ localise: client.localise.bind(client), locale: interaction.displayLocale });
    ...
    const sourceEmbed: Discord.CamelizedDiscordEmbed = {
        ...
        footer: {
            text: strings.sourcedResponsibly({
                dictionaries: client.pluralise("word.strings.sourcedResponsibly.dictionaries", interaction.displayLocale, { quantity: entry.sources.length }),
            }),
        },
    };
    ...
    const entriesStringified = entries.map((entry, indexZeroBased) => {
        ...
        if (isDefinition(entry, entryType)) {
            if (entry.relations !== undefined) {
                const strings = constants.contexts.wordRelations({ localise: client.localise.bind(client), locale: interaction.displayLocale });
...
function fitTextToFieldSize(
    ...
    const strings = constants.contexts.definitionsOmitted({ localise: client.localise.bind(client), locale: interaction.displayLocale });
    const characterOverhead =
        strings.definitionsOmitted({
            definitions: client.pluralise("word.strings.definitionsOmitted.definitions", interaction.displayLocale, { quantity: textParts.length }),
            flag: "verbose",
        }).length + 20;
    ...
    if (stringsOmitted !== 0) {
        fittedString += `\n*${strings.definitionsOmitted({
            definitions: client.pluralise("word.strings.definitionsOmitted.definitions", interaction.displayLocale, { quantity: stringsOmitted }),
            flag: "verbose",
        })}*`;
    }
